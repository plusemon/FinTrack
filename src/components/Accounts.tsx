  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-bold tracking-tight">{t.financialAccounts}</h3>
          <p className="text-zinc-500">{t.manageAccounts}</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {accounts.map((account) => (
          <AccountCard key={account.id} account={account} currency={currency} t={t} />
        ))}
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 relative">
            <h3 className="text-xl font-bold mb-6">{t.newAccount}</h3>
            <AccountForm t={t} onClose={() => { setIsModalOpen(false); fetchAccounts(); }} />
          </div>
        </div>
      )}
    </div>
...
function AccountCard({ account, currency, t }: { account: Account, currency: string, t: any }) {
  return (
    <div className="bg-white p-6 rounded-2xl border border-black/5 shadow-sm hover:shadow-md transition-all group">
      ...
      <div className="mt-6 pt-6 border-t border-black/5 flex items-end justify-between">
        <div>
          <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1">{t.currentBalance}</p>
...
function AccountForm({ onClose, t }: { onClose: () => void, t: any }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-1">
        <label className="text-xs font-bold text-zinc-500 uppercase">{t.accountName}</label>
        <input
          type="text"
          value={name}
      ...
      <div className="space-y-1">
        <label className="text-xs font-bold text-zinc-500 uppercase">{t.accountType}</label>
        <select
          value={type}
      ...
      <div className="space-y-1">
        <label className="text-xs font-bold text-zinc-500 uppercase">{t.initialBalance}</label>
        <input
          type="number"
          value={balance}
