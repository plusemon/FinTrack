  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-bold tracking-tight">{t.budgetManagement}</h3>
          <p className="text-zinc-500">{t.setLimits}</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {budgets.map((budget) => (
          <BudgetCard key={budget.id} budget={budget} currency={currency} t={t} />
        ))}
        {budgets.length === 0 && (
          <div className="col-span-full bg-white p-12 rounded-2xl border border-dashed border-zinc-200 text-center">
            ...
            <p className="text-zinc-500 font-medium">{t.noBudgets}</p>
            <p className="text-zinc-400 text-sm">{t.startSettingBudget}</p>
          </div>
        )}
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 relative">
            <h3 className="text-xl font-bold mb-6">{t.setNewBudget}</h3>
            <BudgetForm 
...
function BudgetCard({ budget, currency, t }: { budget: Budget, currency: string, t: any }) {
    ...
    <div className="bg-white p-6 rounded-2xl border border-black/5 shadow-sm space-y-6">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-bold text-zinc-900">{budget.category_name || t.globalBudget}</h4>
      ...
      <div className="flex justify-between items-end">
        <div>
          <p className="text-xs text-zinc-400 font-bold uppercase mb-1">{t.spent}</p>
          <p className="text-xl font-bold">{formatCurrency(budget.spent || 0, currency)}</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-zinc-400 font-bold uppercase mb-1">{t.budget}</p>
...
function BudgetForm({ onClose, categories, t }: { onClose: () => void, categories: Category[], t: any }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-1">
        <label className="text-xs font-bold text-zinc-500 uppercase">{t.budgetAmount}</label>
        <input
          type="number"
          value={amount}
      ...
      <div className="space-y-1">
        <label className="text-xs font-bold text-zinc-500 uppercase">{t.period}</label>
        <select
          value={period}
